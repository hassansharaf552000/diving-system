...
export class RepCommission {
  ...
  invoiceFields = [
    'fromDate', 'toDate', 'agentId', 'repId', 'reportType'
  ];
  ...
  handleView(cleanedFilters: any) {
    this.downloadReport('/api/RepCommissionReport/pdf', true, cleanedFilters);
  }

  handlePdf(cleanedFilters: any) {
    this.downloadReport('/api/RepCommissionReport/pdf', false, cleanedFilters);
  }

  handleExcel(cleanedFilters: any) {
    this.downloadReport('/api/RepCommissionReport/excel', false, cleanedFilters);
  }
    ...
    this.reportService.downloadReport(endpoint, filtersPayload).subscribe({
      next: (blob: Blob) => {
        ...
        if (openInNewTab) {
          ...
          a.href = url;
          let filename = 'RepCommissionReport' + (endpoint.endsWith('excel') ? '.xlsx' : '.pdf');
          a.download = filename;
