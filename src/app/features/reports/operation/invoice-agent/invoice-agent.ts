...
@Component({
  ...
  standalone: false,
  templateUrl: './invoice-agent.html',
  styleUrl: './invoice-agent.scss',
})
export class InvoiceAgent {
  ...
  handleView(cleanedFilters: any) {
    this.downloadReport('/api/Report/pdf', true, cleanedFilters);
  }

  handlePdf(cleanedFilters: any) {
    this.downloadReport('/api/Report/pdf', false, cleanedFilters);
  }

  handleExcel(cleanedFilters: any) {
    this.downloadReport('/api/Report/excel', false, cleanedFilters);
  }
    ...
    this.reportService.downloadReport(endpoint, filtersPayload).subscribe({
      next: (blob: Blob) => {
        ...
        if (openInNewTab) {
          ...
          a.href = url;
          let filename = 'InvoiceAgentReport' + (endpoint.endsWith('excel') ? '.xlsx' : '.pdf');
          a.download = filename;
