...
export class CodeHotelComponent implements OnInit {
  items: Hotel[] = []; model: Hotel = { hotelName: '' }; isModalOpen = false; isEdit = false; searchTerm = '';
  ...
  ngOnInit(): void { this.loadData(); this.svc.getHotelDestinations().subscribe(d => this.destinations = d); }
  loadData(): void { this.svc.getHotels().subscribe(d => this.items = d); }
  ...
  openAdd(): void { this.model = { hotelName: '' }; this.isEdit = false; this.isModalOpen = true; }
  ...
  save(): void {
    ...
    if (this.isEdit && this.model.id) {
      this.svc.updateHotel(this.model.id, this.model).subscribe(() => { this.loadData(); this.closeModal(); });
    } else {
      this.svc.createHotel(this.model).subscribe(() => { this.loadData(); this.closeModal(); });
    }
  }
