...
export class CodeAgentComponent implements OnInit {
  ...
  save(): void {
    if (!this.model.agentName) { Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 4000, icon: 'warning' }).fire('⚠️ Please fill in Agent Name'); return; }
    this.saving = true;
